...
const update = await __TAURI__.updater.check();
if (update) {
  console.log(
  ...
  await update.downloadAndInstall((event) => {
    switch (event.event) {
      case 'Started':
        ...
        status.innerHTML = `Downloading update... (${Math.floor(100 * (downloaded / contentLength))}%)`;
        break;
      case 'Progress':
  ...
  await __TAURI__.process.relaunch();
    ...
    __TAURI__.deepLink.getCurrent().then(urls => {
      console.log(urls ? JSON.stringify(urls) : "undefined");
